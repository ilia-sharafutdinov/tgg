...
shutil.copytree(dist_dir, temp_dir, ignore=shutil.ignore_patterns('README.md', 'patch6.pack', 'sounds_sfx.pack', 'tgg_media_*', '*.zip'))
...
if os.path.isdir(temp_dir):
    shutil.rmtree(temp_dir)
shutil.copytree(dist_dir, temp_dir, ignore=shutil.ignore_patterns('README.md', 'patch6.pack', '*.zip'))
